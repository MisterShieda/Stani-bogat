#ifndef CRYPT_H
#define CRYPT_H

#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define AES_BLOCK_SIZE 16
#define Nb 4
#define Nk 4
#define Nr 10

static const uint8_t sbox[256] = {
    
};

static const uint8_t rsbox[256] = {
    
};

static const uint8_t Rcon[11] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
};

static const uint8_t aes_key[16] = {
    0x2b,0x7e,0x15,0x16,0x28,0xae,0xd2,0xa6,
    0xab,0xf7,0x70,0x09,0xcf,0x4f,0x3c,0x6a
};

static void SubBytes(uint8_t* state) {
    for (int i = 0; i < 16; ++i)
        state[i] = sbox[state[i]];
}

static void InvSubBytes(uint8_t* state) {
    for (int i = 0; i < 16; ++i)
        state[i] = rsbox[state[i]];
}

static void ShiftRows(uint8_t* state) {
    uint8_t temp;

    temp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = temp;
    temp = state[2]; state[2] = state[10]; state[10] = temp;
    temp = state[6]; state[6] = state[14]; state[14] = temp;
    temp = state[3]; state[3] = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = temp;
}

static void InvShiftRows(uint8_t* state) {
    uint8_t temp;

    temp = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = temp;
    temp = state[2]; state[2] = state[10]; state[10] = temp;
    temp = state[6]; state[6] = state[14]; state[14] = temp;
    temp = state[3]; state[3] = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = temp;
}

static uint8_t xtime(uint8_t x) {
    return ((x << 1) ^ (((x >> 7) & 1) * 0x1b));
}

static void MixColumns(uint8_t* state) {
    for (int i = 0; i < 4; ++i) {
        uint8_t* col = state + i * 4;
        uint8_t t = col[0] ^ col[1] ^ col[2] ^ col[3];
        uint8_t tmp = col[0];
        col[0] ^= t ^ xtime(col[0] ^ col[1]);
        col[1] ^= t ^ xtime(col[1] ^ col[2]);
        col[2] ^= t ^ xtime(col[2] ^ col[3]);
        col[3] ^= t ^ xtime(col[3] ^ tmp);
    }
}

static void InvMixColumns(uint8_t* state) {
    for (int i = 0; i < 4; ++i) {
        uint8_t* col = state + i * 4;
        uint8_t a = col[0], b = col[1], c = col[2], d = col[3];
        col[0] = xtime(xtime(xtime(a ^ c))) ^ xtime(xtime(b ^ c)) ^ xtime(d ^ b) ^ a;
        col[1] = xtime(xtime(xtime(b ^ d))) ^ xtime(xtime(c ^ d)) ^ xtime(a ^ c) ^ b;
        col[2] = xtime(xtime(xtime(c ^ a))) ^ xtime(xtime(d ^ a)) ^ xtime(b ^ d) ^ c;
        col[3] = xtime(xtime(xtime(d ^ b))) ^ xtime(xtime(a ^ b)) ^ xtime(c ^ a) ^ d;
    }
}

static void AddRoundKey(uint8_t* state, uint8_t* roundKey) {
    for (int i = 0; i < 16; ++i)
        state[i] ^= roundKey[i];
}

static void KeyExpansion(const uint8_t* key, uint8_t* roundKeys) {
    memcpy(roundKeys, key, 16);
    uint8_t temp[4];
    int i = 4;
    while (i < 4 * (Nr + 1)) {
        memcpy(temp, &roundKeys[4 * (i - 1)], 4);
        if (i % Nk == 0) {
            uint8_t t = temp[0];
            temp[0] = sbox[temp[1]] ^ Rcon[i / Nk];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[t];
        }
        for (int j = 0; j < 4; j++)
            roundKeys[4 * i + j] = roundKeys[4 * (i - Nk) + j] ^ temp[j];
        i++;
    }
}

static void AES_encrypt_block(uint8_t* input, const uint8_t* key) {
    uint8_t state[16], roundKeys[176];
    memcpy(state, input, 16);
    KeyExpansion(key, roundKeys);
    AddRoundKey(state, roundKeys);

    for (int round = 1; round < Nr; ++round) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, roundKeys + round * 16);
    }

    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, roundKeys + Nr * 16);
    memcpy(input, state, 16);
}

static void AES_decrypt_block(uint8_t* input, const uint8_t* key) {
    uint8_t state[16], roundKeys[176];
    memcpy(state, input, 16);
    KeyExpansion(key, roundKeys);
    AddRoundKey(state, roundKeys + Nr * 16);

    for (int round = Nr - 1; round > 0; --round) {
        InvShiftRows(state);
        InvSubBytes(state);
        AddRoundKey(state, roundKeys + round * 16);
        InvMixColumns(state);
    }

    InvShiftRows(state);
    InvSubBytes(state);
    AddRoundKey(state, roundKeys);
    memcpy(input, state, 16);
}

void encrypt(const char* filename) {
    FILE* f = fopen(filename, "rb+");
    if (!f) { perror("encrypt fopen"); return; }

    fseek(f, 0, SEEK_END);
    long len = ftell(f);
    rewind(f);

    long padded_len = ((len + AES_BLOCK_SIZE - 1) / AES_BLOCK_SIZE) * AES_BLOCK_SIZE;
    uint8_t* buffer = (uint8_t*)calloc(1, padded_len);
    fread(buffer, 1, len, f);

    for (long i = 0; i < padded_len; i += AES_BLOCK_SIZE)
        AES_encrypt_block(buffer + i, aes_key);

    rewind(f);
    fwrite(buffer, 1, padded_len, f);
    fclose(f);
    free(buffer);
}

void decrypt(const char* filename) {
    FILE* f = fopen(filename, "rb+");
    if (!f) { perror("decrypt fopen"); return; }

    fseek(f, 0, SEEK_END);
    long len = ftell(f);
    rewind(f);

    uint8_t* buffer = (uint8_t*)malloc(len);
    fread(buffer, 1, len, f);

    for (long i = 0; i < len; i += AES_BLOCK_SIZE)
        AES_decrypt_block(buffer + i, aes_key);

    rewind(f);
    fwrite(buffer, 1, len, f);
    fclose(f);
    free(buffer);
}

#endif